...
#define AP_DRIVER_VERSION			"4.4.2.0"
...
#define STA_DRIVER_VERSION			"4.4.1.0"
#ifdef MULTIPLE_CARD_SUPPORT
...
#define BF_GAIN_TABLE_FILE_NAME     "/etc/Wireless/RT2870STA/BF_Gain_Table.dat"
#define BF_SKU_TABLE_FILE_NAME      "/etc/Wireless/RT2870STA/SingleSKU_BF.dat"
#endif /* SINGLE_SKU_V2 */
...
/* 
    rcu_read_lock/rcu_read_unlock is one of GPL APIs in preemptive kernel 
    and actually we don't use the way of PID to do something anymore. 
*/
...
#ifdef RTMP_SDIO_SUPPORT
//#if CFG_SDIO_TX_THREAD
	RTMP_NET_TASK_STRUCT SdioRxdonetask;
//#endif
#endif
...
do{	\
	...
	if (Status != 0)	\
	{	\
		MTWF_LOG(DBG_CAT_HIF, CATHIF_USB, DBG_LVL_ERROR, ("%s, line(%d), CR Read Error\n", __FUNCTION__, __LINE__));	\
	}\
} while(0)
...
do{	\
	...
	if (Status != 0)	\
	{	\
		MTWF_LOG(DBG_CAT_HIF, CATHIF_USB, DBG_LVL_ERROR, ("%s, line(%d), CR Write Error\n", __FUNCTION__, __LINE__));	\
	}\
} while(0)
