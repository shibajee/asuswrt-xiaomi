...
#if defined (MTK_SD_RESCUE)
#if defined (CFG_ENV_IS_IN_NAND) || defined (CFG_ENV_IS_IN_SPI)
		case 'a':
			printf("\n%d: System Load Linux then write to Flash via SD Card Storage. \n", 'a');
            /*
            argc=3;
            argv[1] = "mmc";
            argv[2] = "0:1";
            if(do_fat_fsinfo(cmdtp,0,argc,argv)){
                printf("fail to query fatinfo\n");
            }*/
            ...
            argv[4] = "root_uImage";
	        if(do_fat_fsload(cmdtp, 0, argc, argv)){
				printf("Upgrade F/W from SD Card storage failed.\n");
				break;
			}

			NetBootFileXferSize=simple_strtoul(getenv("filesize"), NULL, 16);
            printf("Running SD rescuing ... wait for a moment \n");
#if defined (CFG_ENV_IS_IN_NAND)
