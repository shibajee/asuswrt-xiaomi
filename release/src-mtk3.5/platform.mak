...
export LINUXDIR := $(SRCBASE)/linux/linux-3.10.108
...
define platformKernelConfig
	...
	if [ "$(REPEATER)" = "y" ] ; then \
		...
		if [ "$(FIRST_IF)" = "MT7615E" ] || [ "$(SECOND_IF)" = "MT7615E" ] || [ "$(SECOND_IF)" = "MT7663E" ]; then \
			sed -i "/CONFIG_APCLI_SUPPORT/d" $(1); \
			echo "CONFIG_APCLI_SUPPORT=y" >>$(1); \
			sed -i "/CONFIG_MAC_REPEATER_SUPPORT/d" $(1); \
		...
		if [ "$(FIRST_IF)" = "MT7628" ]; then \
			sed -i "/CONFIG_MT7628_APCLI_SUPPORT/d" $(1); \
			echo "CONFIG_MT7628_APCLI_SUPPORT=y" >>$(1); \
			sed -i "/CONFIG_MT7628_MAC_REPEATER_SUPPORT/d" $(1); \
			echo "CONFIG_MT7628_MAC_REPEATER_SUPPORT=y" >>$(1); \
			sed -i "/CONFIG_MT7628_APCLI_CERT_SUPPORT/d" $(1); \
			echo "CONFIG_MT7628_APCLI_CERT_SUPPORT=y" >>$(1); \
		fi; \
		if [ "$(RPAC87)" = "y" ] ; then \
			sed -i "/CONFIG_RA_NAT_HW/d" $(1); \
